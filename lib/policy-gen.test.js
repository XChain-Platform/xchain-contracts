...
//
// Two layers:
//   1. Config validation + lint: every generated guard must pass the deploy-time
//      linter (xchain-vm lint-core, pure acorn) with ZERO errors and ZERO
//      warnings. Runs on any Node (the linter is dependency-light); the lint
//      block skips gracefully if the adjacent xchain-vm checkout is absent.
...
//      enforcement logic actually runs, WITHOUT isolated-vm (which is Linux-only
//      here), so it runs on any Node too.
//
...
// A representative matrix of configs used by both the lint and behaviour blocks.
const MATRIX = {
