...
// against the other side, which skips every lint assertion in this file while
// the suite still reports green, so try the post-move spelling and fall back to
// the pre-move one.
...
describe('policy-gen: config validation', function () {
    ...
    // Local tripwire on the generator's OWN list: an unreviewed edit to
    // ACTION_CLASSES lands here. It is NOT a cross-repo drift guard and must not
    // be read as one. This suite is sibling-light and VM-free and never opens
    // xchain-indexer/src/config.js, so an indexer-side change leaves it green -
    // which is the 'ownership' drift it was once claimed to have caught (#6961).
    // The comparison against the indexer's canonical CONTROLLER_BINDABLE_CLASSES
    // and against the SDK's two copies runs at the platform venue, in
    // bin/check-controller-class-parity.js; that gate also fails if the literal
    // below stops carrying the canonical set, so deleting this pin is not a way
    // around it.
    ...
    // A royalty leg "to" is the one config value the CHAIN re-validates: the
    // indexer decodes every returned leg and denies the whole trade-class action
    // with 'controller (bad payout leg)' if one fails, so a bad leg does not lose
    // the royalty, it delists the token. The gate is network-agnostic on purpose
    // (any version byte, any HRP), which makes it a strict superset of the chain
    // rule; the testnet, DOGE and bech32 accept-cases below are what pin that.
    ...
    it('accepts real base58check and bech32 leg addresses on any network', function () {
        const good = ['1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa',         // mainnet P2PKH
                      '3J98t1WpEZ73CNmQviecrnyiWrnqRhWNLy',         // mainnet P2SH
                      'mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn',         // testnet P2PKH
                      'DH5yaieqoZN36fDVciNyRueRGvGLR3mr7L',         // DOGE P2PKH
                      'bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4', // bech32 v0
                      'bc1pw508d6qejxtdg4y5r3zarvary0c5xw7kw508d6qejxtdg4y5r3zarvary0c5xw7kt5nd6y']; // bech32m
    ...
    it('the leg-address rule does NOT reach owner, freeze or allowlist entries', function () {
        // Those are string-equality comparisons inside the guard, and a contract
        // ledger address is a legitimate holder there.
...
describe('policy-gen: generated guard behaviour (mock xchain)', function () {
    ...
    // DISPENSER_CREATE is in the trade class too, so the guard runs on it, but the
    // dispenser handler discards payoutLegs and dispense applies no split. The guard
    // allows it with no legs today; pinned so a change here is a deliberate one, and
    // disclosed in the header and bind hints so a deployer learns it before binding.
...
// `meta` is the one generated key consensus actually reads: the indexer refuses a
// DEPLOY whose contract exports no meta.name / meta.description. A no-code path that
// emitted a guard without it would hand every Tier 0 user an invalid deploy they paid
// a fee for, so these assert the DEFAULTS exist and are honest, not just that the key
// is present. Loaded through loadContract (real parse, real object) rather than
// string-matched, so a syntactically broken block fails here.
describe('policy-gen: contract identity (meta)', function () {
    ...
    it('every matrix variant emits meta as the FIRST exported key, fully populated', function () {
        for (const key of Object.keys(MATRIX)) {
            ...
            // The consensus caps, in bytes, checked on the emitted value not the input.
    ...
    it('the default name is the config name and the default description names the gates and the rules', function () {
        ...
        assert.strictEqual(c.meta.description,
            ...
            'trade-class actions.');
        // Singular vs plural, and only the rules actually enabled.
        ...
        // The resolved allowlist direction is in the sentence, not just the descriptor.
    ...
    it('a caller-supplied meta wins, field by field, over the generated defaults', function () {
        ...
        // A partial meta overrides only what it names; the rest still generates.
    ...
    it('rejects a meta that consensus would reject, rather than emitting an undeployable guard', function () {
        ...
        // Deliberately narrower than the consensus grammar, which does admit non-ASCII.
    ...
    it('a name whose edge spaces the header tolerates is trimmed for meta, not rejected', function () {
        // NAME_RE admits '  Spaced  '; the meta grammar does not. A config that generated
        // a clean guard before meta existed must still generate one.
